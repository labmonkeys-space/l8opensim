...
- **Shipped vendor catalogs** (PRs 4 & 5 of epic #103):
  ...
  - `juniper_mx240/traps.json` — ships in PR 5.
  - Other cisco_* slugs (`cisco_catalyst_9500`, `cisco_crs_x`, `cisco_nexus_9500`, `asr9k`) fall back to universal in this epic. Family-catalog concept is a follow-up refactor.
...
- **Shipped vendor catalogs** (PRs 4 & 5 of epic #103):
  - `cisco_ios/syslog.json` — 7 Cisco-format messages: `%LINK-3-UPDOWN` (up/down pair), `%LINEPROTO-5-UPDOWN`, `%SYS-5-CONFIG_I`, `%SNMP-5-COLDSTART`, `%SYS-5-RESTART`, `%ENVMON-5-TEMP_OK`. Merged with universal 6 → cisco_ios devices fire from 13 entries. Message bodies match IOS's `%FACILITY-SEVERITY-MNEMONIC:` form verbatim so OpenNMS UEI matchers tuned for Cisco strings fire correctly.
  - `juniper_mx240/syslog.json` — ships in PR 5.
